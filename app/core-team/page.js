...
const CoreTeamMemberCard = ({ imgSrc, name, title }) => (
  <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 text-center hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
    <img src={imgSrc} alt={name} className="w-64 h-64 rounded-full mx-auto mb-4 border-4 border-white shadow-md" />
    <h3 className="text-3xl font-bold text-gray-800">{name}</h3>
    <p className="text-orange-600 font-semibold mt-1 text-xl">{title}</p>
  </div>
);
...
const ValueCard = ({ icon, title, children, color }) => {
  ...
  return (
    <div className="flex items-center bg-white p-8 rounded-2xl shadow-lg border border-gray-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
